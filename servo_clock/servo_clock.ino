...
*/
// ********* НАСТРОЙКИ *********
#define SERVOMIN  220     // минимальный сигнал серво (Новые значения работают почти на 70% серво)
#define SERVOMAX  600     // максимальный сигнал серво
...
#define N_SERVOS 28       // Количество серво, если хотите как-то изменить часы.
...
uint8_t servoInterval = 15; // временной интервал приращения угла для плавного поворота.
// Взят 15мс, так как при повороте ВСЕХ серво этот блок выполняется 18 мс
// 8 бит достаточно, так как выше 255 не идёт.
...
uint16_t angle[N_SERVOS];       // личный угол каждой серво
uint16_t new_angle[N_SERVOS];   // новый угол. Меняем значение массива - серво поворачивается
uint16_t up_angle[N_SERVOS];    // угол, при котором индикатор поднят. Хранится в EEPROM
boolean clock_state;
